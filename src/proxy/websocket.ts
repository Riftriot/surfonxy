...
import { SURFONXY_URI_ATTRIBUTES, SURFONXY_WEBSOCKET_PATH_PREFIX } from "~/utils/constants";

import TCPWebSocket from "tcp-websocket";
export const webSocketProxyHandler: WebSocketHandler<{
  request: Request,
  ...
  open (ws) {
    ...
    const original_pathname = patched_url.pathname.slice(SURFONXY_WEBSOCKET_PATH_PREFIX.length);
...
export type WebSocketElysiaProxyHandler = Omit<Partial<WebSocketHandler<Context>>, "publish" | "open" | "message" | "close" | "drain" | "publishToSelf">
